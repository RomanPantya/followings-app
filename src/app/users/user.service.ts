...
export class UserService {
  ...
  async getAllInfoForUser(userId: number, options: GetUserByIdQueryDto) {
    ...
    const friends = followings
      .filter(({ id: followingId }: UserEntity) => followers
      .some(({ id: followerId }: UserEntity) => followingId === followerId))
      .sort((u1: UserEntity, u2: UserEntity) => {
        if (['email', 'gender', 'first_name'].includes(order_by)) {
          return order_type === OrderByEnum.ASC
          ? 
          (u1[order_by as ['email', 'gender', 'first_name'][number]] || '')
          .localeCompare((u2[order_by as ['email', 'gender', 'first_name'][number]] || ''))
          
          : 
          (u2[order_by as ['email', 'gender', 'first_name'][number]] || '')
          .localeCompare((u1[order_by as ['email', 'gender', 'first_name'][number]] || ''));
        }
...
`)
  ...
  getTop5() {
    return this.usersRepository.query(`
      select u.*, (count(f.user_id)) followers from users u
      join followings f
      on u.id = f.follower_id
      group by id
